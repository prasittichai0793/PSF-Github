...
<head>
    ...
    <title>C เพิ่มข้อมูลพนักงานขับรถโฟล์คลิฟ</title>
    ...
    <div class="display-container">
        <div class="display-content">
            <div class="container-content">
                <div class="content">
                    <span>C เพิ่มข้อมูลพนักงานขับรถโฟล์คลิฟ</span>
                    <form action="add_pos3.php" method="post">
                        ...
                        <button type="button" class="btn btn-cancel" onclick="location.href='show_position3.php'"><i class="fas fa-times"></i>ยกเลิก</button>
                    </form>
                </div>
            </div>
        </div>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // ดึง user_no ล่าสุด
    $sql_last_no = "SELECT user_no FROM tb_user WHERE user_no LIKE 'C_%' ORDER BY user_no DESC LIMIT 1";
    ...
    if ($result->num_rows > 0) {
        ...
        // เพิ่มลำดับเลขต่อไป
        $new_no = 'C_' . str_pad($last_no_int + 1, 3, '0', STR_PAD_LEFT);
    } else {
        $new_no = 'C_001';
    }
    ...
    $sql = "INSERT INTO tb_user (user_no, user_name, user_startDate, user_exp, user_idNumber, user_phoneNumber, user_Date, user_age, user_day, user_username, user_password, user_gender, position_id)
            VALUES ('$new_no', '$user_name', '$formatted_startDate', '$user_exp', '$user_idNumber', '$user_phoneNumber', '$formatted_user_age', '$user_age', '$user_leaveDays', '$user_username', '$user_password', '$user_gender', '3')";

    if ($conn->query($sql) === TRUE) {
        echo "<script>alert('Successful - New user No: $new_no'); window.location.href = 'show_position3.php';</script>";
        exit();
    } else {
