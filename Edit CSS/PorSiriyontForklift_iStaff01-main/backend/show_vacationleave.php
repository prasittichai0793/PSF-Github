...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['approve'])) {
        ...
        $sql = "SELECT VLeave_dateStart, VLeave_dateEnd, user_id FROM tb_vacationleave WHERE VLeave_id='$vleave_id'";
        $result = $conn->query($sql);
        
        if ($result->num_rows > 0) {
            $row = $result->fetch_assoc();
            $vleave_dateStart = $row['VLeave_dateStart'];
            $vleave_dateEnd = $row['VLeave_dateEnd'];
            $user_id = $row['user_id'];

            // คำนวณจำนวนวันที่ลา
            $dateStart = new DateTime($vleave_dateStart);
            $dateEnd = new DateTime($vleave_dateEnd);
            ...
            $sqlUpdate = "UPDATE tb_vacationleave SET VLeave_status='อนุมัติ', hr_id='$hr_id' WHERE VLeave_id='$vleave_id'";
        }
    } elseif (isset($_POST['reject'])) {
        // อัปเดตสถานะการลาเป็น 'ไม่อนุมัติ' แต่ไม่ลดวันลา
        $sqlUpdate = "UPDATE tb_vacationleave SET VLeave_status='ไม่อนุมัติ', hr_id='$hr_id' WHERE VLeave_id='$vleave_id'";
    }
