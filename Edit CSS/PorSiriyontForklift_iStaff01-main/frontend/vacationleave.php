...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $doc_path = null;
    $proceed_with_insert = true; // เพิ่มตัวแปรควบคุมการทำงาน

    if (isset($_FILES['document']) && $_FILES['document']['error'] == 0) {
        $upload_dir = "../uploads/vacationleave_docs/";
        ...
        window.location.href='vacationleave.php';
    </script>";
            exit();
        }
    ...
    // ดำเนินการบันทึกข้อมูลเมื่อไม่มีปัญหา
    $insert_query = "INSERT INTO tb_vacationleave (user_id, VLeave_dateStart, VLeave_dateEnd, 
        VLeave_detail, VLeave_docs, VLeave_status, VLeave_date) 
        VALUES (?, ?, ?, ?, ?, ?, ?)";
